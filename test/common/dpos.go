...
// ChangeCandidate
func (acc *Account) ChangeCandidate(to common.Name, value *big.Int, id uint64, gas uint64, candidate string) []byte {
	arg := &args.ChangeCandidate{
		Candidate: candidate,
	}
	payload, err := rlp.EncodeToBytes(arg)
	if err != nil {
	...
	action := types.NewAction(types.ChangeCandidate, acc.name, to, acc.nonce, id, gas, value, payload)
	if acc.getnonce == nil {
		acc.nonce++
	}
...
func (acc *Account) UnvoteCandidate(to common.Name, value *big.Int, id uint64, gas uint64) []byte {
	if acc.getnonce != nil {
		acc.nonce = acc.getnonce(acc.name)
	}
	action := types.NewAction(types.UnvoteCandidate, acc.name, to, acc.nonce, id, gas, value, nil)
	if acc.getnonce == nil {
		acc.nonce++
	}
